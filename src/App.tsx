...
import { User, Search, Plus, Lock, Shield, X } from 'lucide-react';
  ...
  if (!unlocked) return (
    <div className="min-h-screen flex items-center justify-center p-4" style={{backgroundColor: '#0a0a0f'}}>
      <div className="w-full max-w-md text-center">
        <div className="inline-flex items-center justify-center w-20 h-20 rounded-2xl mb-6" style={{background: '#4f46e520', border: '1px solid #4f46e540'}}>
          <User className="w-9 h-9" style={{color: '#4f46e5'}} />
        </div>
        <h1 className="text-3xl font-bold text-white mb-2">CVBuilder Pro</h1>
        ...
        <form onSubmit={unlock} className="space-y-4">
          ...
          <button type="submit" className="w-full text-white font-semibold py-3.5 rounded-xl transition-colors" style={{backgroundColor: '#4f46e5'}}>
            Unlock
          </button>
        </form>
    ...
    <div className="min-h-screen" style={{backgroundColor: '#0a0a0f'}}>
      <header className="border-b border-[#1e1e2e] px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <User className="w-6 h-6" style={{color: '#4f46e5'}} />
          <span className="font-bold text-white">CVBuilder Pro</span>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => setShowAdd(true)} className="flex items-center gap-2 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors" style={{backgroundColor: '#4f46e5'}}>
          ...
          <div className="bg-[#111118] border border-[#1e1e2e] rounded-2xl p-6 w-full max-w-md">
            ...
            <div className="flex gap-2">
              <button onClick={() => { if (newItem.trim()) { setItems(prev => [newItem.trim(), ...prev]); setNewItem(''); setShowAdd(false); } }}
                className="flex-1 text-white font-semibold py-3 rounded-xl transition-colors" style={{backgroundColor: '#4f46e5'}}>Add</button>
